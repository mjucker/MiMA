...
!epg: we now pass cg_drag the local latitudinal structure, in rad_lat,
!     instead of the global latb array, latb.  Note that this might 
!     not work if column_diagnostics are used!
!subroutine cg_drag_init (lonb, latb, pref, Time, axes)
subroutine cg_drag_init (lonb, rad_lat, pref, Time, axes)
...
real,    dimension(:), intent(in)      :: lonb, rad_lat, pref
...
!       rad_lat   array of model latidudes at cell centers [radians]
...
!-------------------------------------------------------------------
      kmax = size(pref(:)) - 1
      !epg: we now pass a local array for latitude.  It's okay to use
      !     the global lonb array because paralleization is done in
      !     the meridional, at least for the spectral model!!!
      !jdf  = size(latb(:)) - 1
      jdf  = size(rad_lat(:))
      idf  = size(lonb(:)) - 1
...
!wfc++ new code
      do j=1,jdf
         !epg: modified to account for new input rad_lat
...
!!          eps = (Bt_0*1.5/nk)/Bsum
          ! epg: we'll omit the "unexplained" factor of 1.5
