...
! epg: something here was interfering with the tracers.  I'll write out
!      the tendencies through atmosphere.f90
...
!epg: I'll comment out the SKIP notations ...
...
!epg: I've added the last 2 subroutines (cg_drag_time_vary and _endts) to 
!     increment the cg_drag_alarm.  This follows the riga (AM3) version of the code
...
!epg: needed for the cg_drag_alarm capability
...
integer     :: cg_drag_freq=0     ! calculation frequency [ s ]
                                  ! epg: note this is set to 1800 in CM3 runs
...
! epg: what are these for ... need to revisit this 
...
!---------------------------------------------------------------------
!   epg: scheme diagnostic parameters (we will likely write our own
!        diagnostics, and so not use these ....)
!---------------------------------------------------------------------
logical     :: calculate_ked=.false.  ! calculate ked diagnostic ?
...
!epg: a flag to turn off cg_drag
logical   ::  no_cg_drag = .true.
...
namelist / cg_drag_nml /         &
                          ...
                          phi0n,phi0s,dphin,dphis, no_cg_drag
...
!---------------------------------------------------------------------
!   wave spectrum parameters.
!   epg: I moved the free parameters to the namelist
!---------------------------------------------------------------------
...
!epg: following the riga (AM3) code, we using a different strategy to
!     handle the alarm.  pts_processed and total_pts are no longer
!     used
!---------------------------------------------------------------------
...
subroutine cg_drag_init (lonb, latb, cg_rad_lat, pref, Time, axes)
...
!-------------------------------------------------------------------
! epg: real,    dimension(:), intent(in)      :: lonb, latb, pref
real,    dimension(:), intent(in)      :: lonb, latb, cg_rad_lat, pref
...
!  epg: cg_rad_lat array of model latitudes ot cell centers [radians]   
...
!epg: I'm cutting out any direct output
!      call diag_manager_init
      call constants_init
!epg: I've removed this (it was part of the old "SKIP" cut
!wfc++ remove for general use
...
!epg: are you meaning to use cg_drag?
...
!epg: actually, this part of the code was a bit confused.  idf is both global
!     and local (at least for the spectral model, since each processor has all 
!     longitudes)
!     jdf should be local, but we also need a global value, j_global, for defining
!     the gwd_u and _v arrays
      kmax = size(pref(:)) - 1
      j_global  = size(latb(:)) - 1
      jdf  = size(cg_rad_lat)
      idf  = size(lonb(:)) - 1
...
!rjw                             (Bt_nh-Bt_sh)*(1.0+tanh(4.0*lat(i,j)))/2.0 + Bt_sh
            ...
            source_amp(i,j) = Bt_0 +                         &
                 Bt_nh*0.5*(1.+tanh((cg_rad_lat(j)/pif-phi0n)/dphin)) + &
                 Bt_sh*0.5*(1.+tanh((cg_rad_lat(j)/pif-phi0s)/dphis))
         end do
      end do
...
!epg: debugging
!print *, mpp_pe(), cg_rad_lat
!print *, mpp_pe(), source_amp(1,:)  
!print *, mpp_pe(), source_level(1,:)  
...
!epg: I'm removing any direct output
!      id_bf_cgwd =  &
...
!epg: removing any direct output
!        id_kedx_cgwd =  &
...
! epg: I will set up the code to automatically compute the cg_drag 
!      upon initialization, and not save information afterwards for
!      restarts.  This means that it's important the cg_drag_freq
!      divide 86400 (or at least comes to a clean finish at the end
!      of each integration.  
      cgdrag_alarm = 0
! I am following the structure in the riga code, here; we need to save 
! both the u and v tendencies
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
! epg: this is where we got in trouble.  These are global arrays, as 
!      opposed to source_amp and _level
!      allocate ( gwd_u(idf,jdf,kmax) )
!      allocate ( gwd_v(idf,jdf,kmax) )
      allocate ( gwd_u(idf,j_global,kmax) )
      allocate ( gwd_v(idf,j_global,kmax) )
! set the save tendencies to zero initially      
      gwd_u(:,:,:) = 0.0
      gwd_v(:,:,:) = 0.0
...
!epg: this subroutine no longer uses lat, so I will remove it.  I've
!     also removed the need for the time step (delt), and will clean
!     that out as well.
!subroutine cg_drag_calc (is, js, lat, pfull, zfull, temp, uuu, vvv,  &
!                         Time, delt, gwfcng_x, gwfcng_y)
subroutine cg_drag_calc (is, js, pfull, zfull, temp, uuu, vvv,  &
...
!wfc--
!epg: not used - real, dimension(:,:),   intent(in)      :: lat
real, dimension(:,:,:), intent(in)      :: pfull, zfull, temp, uuu, vvv
type(time_type),        intent(in)      :: Time
!epg: we no longer need the time step here
!real           ,        intent(in)      :: delt
...
!       lat      epg - not used - array of model latitudes at cell boundaries [radians]
...
!epg: do you mean to use cg_drag?
if (no_cg_drag) return
...
!epg: I will implement the alarm through another subroutine, 
!     cg_drag_time_vary
!---------------------------------------------------------------------
...
!epg: debugging
!      if(mpp_pe() == mpp_root_pe()) print *,'cgdrag_alarm = ', cgdrag_alarm
     
           

!epg: we use the alarm, following the implementation in the riga AM3 version
...
! epg: the change below is consistent with the riga AM3 code
!----------------------------------------------------------------------
...
!epg: we store the forcing so that it can be used at times when it is
!     not computed explicitly.  Here I'm following the riga version 
!--------------------------------------------------------------------
!    store the gravity wave forcing into a processor-global array.
!-------------------------------------------------------------------
...
! epg: we will not use the KE calculation
!--------------------------------------------------------------------
...
!epg: I'm cutting out all diagnostics
...
!epg: here I'm following the riga version ...
!--------------------------------------------------------------------
...
! epg: We use the alarm, but follow the strategy of the riga (AM3) code,
!      and so no longer increment things in this subroutine
!--------------------------------------------------------------------
...
!epg: I'll try to adopt the riga (AM3) strategy, and use a seperate subroutine
!     to increment the cg_drag alarm.  It seems that delt should be an integer,
!     to be in keeping with cgdrag_alarm and cg_drag_freq, which are both
!     integers.  (here I differ from riga/AM3)
...
!####################################################################
! epg: again, we use the strategy in the riga (AM3) code
...
! epg: we will not use restart files
!wfc++ remove for general use
!#ifdef SKIP
!wfc--
...
!    unexplained.
!    epg: given the "unexplained" nature of the 1.5, we've eliminated it
!         We're not sure where it came from, but it is not consistent with
!         A&D 1999.  We increase the GFDL AM3 numbers by 50% to account for
 !        this change
!---------------------------------------------------------------------
          if (Bsum == 0.0) then
            call error_mesg ('cg_drag_mod', &
...
!----------------------------------------------------------------------
                  if (c0mu(k) == 0.) then
                    msk(n) = 0
!epg: this momentum flux is lost.  However, this happens with probability zero,
!     as it only occurs when the wave speed is exactly equal to u.
                  else
...
!epg: force conservation at the model top; all waves must break in
!     the top level, irregardless of the breaking criterion
                      if ( k==0 ) then
                         msk(n) = 0
                         if (k  < iz0) then
                            fm = fm + B0(n)
...
!     deposited in the uppermost layer must be added to the level below
                if ( k==0 ) then
